...
DOCUMENTATION = r"""
---
module: d42_api_appgroup_calc_template
short_description: Manage Device42 application group calculation templates
description:
    - Create, update, or delete Device42 application group calculation templates.
...
options:
    ...
    state:
        description:
            - Desired state of the application group calculation template.
        required: false
        type: str
        default: present
        choices:
    ...
    name:
        description:
            - Application group calculation template name.
            - Used to target existing templates when C(template_id) is not provided.
        required: false
        type: str
    payload:
        description:
            - JSON payload for create/update requests to C(/api/2.0/app_group_calc_template/).
...
EXAMPLES = r"""
- name: Create application group calculation template
    xbh03.device42.d42_api_appgroup_calc_template:
        ...
        payload:
            ...
            doql_query: SELECT servicecommunication_pk FROM view_servicecommunication_v2

- name: Update application group calculation template by ID
    xbh03.device42.d42_api_appgroup_calc_template:
        host: device42.example.internal
        userid: admin
        password: secret123
...
- name: Delete application group calculation template by name
    xbh03.device42.d42_api_appgroup_calc_template:
        host: device42.example.internal
        client_key: abc123
        client_secret_key: secret123
...
api_appgroup_calc_template:
    description: Current or resulting application group calculation template data when available.
    type: dict
    returned: success
response:
...
def _find_template(helper, template_id=None, name=None):
    if template_id is not None:
        return helper.d42_get_appgroup_calc_template_by_id(template_id=template_id)

    if name is None:
        return None

    payload = helper.d42_get_appgroup_calc_templates(name=name)
...
def _build_payload(module):
    ...
    if template_id is not None:
        if payload.get("id") is not None and int(payload.get("id")) != int(template_id):
            module.fail_json(msg="template_id and payload.id reference different application group calculation templates.")
        payload["id"] = int(template_id)

    if name is not None:
        if payload.get("name") is not None and payload.get("name") != name:
            module.fail_json(msg="name and payload.name reference different application group calculation templates.")
        payload["name"] = name
...
def run_module():
    ...
    if state == "absent" and template_id is None and name is None:
        module.fail_json(msg="Either template_id or name is required when state=absent.")

    existing = _find_template(helper=helper, template_id=template_id, name=name)

    if state == "absent":
        if not existing:
            result["msg"] = "Application group calculation template already absent"
            module.exit_json(**result)

        resolved_template_id = existing.get("id")
        if resolved_template_id is None and template_id is not None:
            resolved_template_id = template_id

        result["api_appgroup_calc_template"] = existing
        result["changed"] = True

        if module.check_mode:
            result["msg"] = "Application group calculation template would be deleted"
            module.exit_json(**result)

        response = helper.d42_delete_appgroup_calc_template(template_id=resolved_template_id)
    ...
    if existing is None and merged_payload.get("name") is None:
        module.fail_json(msg="name (or payload.name) is required to create a new calculation template.")
    ...
    if not needs_change:
        result["api_appgroup_calc_template"] = existing
        result["msg"] = "Application group calculation template already in desired state"
        module.exit_json(**result)

    result["changed"] = True
    if module.check_mode:
        result["msg"] = "Application group calculation template would be created/updated"
        if existing:
            result["api_appgroup_calc_template"] = existing
        module.exit_json(**result)

    response = helper.d42_post_appgroup_calc_template(payload=merged_payload)
    result["response"] = response
    ...
    result["api_appgroup_calc_template"] = _find_template(helper=helper, template_id=refresh_id, name=refresh_name)
    module.exit_json(**result)
